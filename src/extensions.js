...
SnapExtensions.primitives.set(
    'scb_putintoslot(item, slot, block)',
    function(thing, slot, target) { // To be cleaned up eventually
//if empty
...
SnapExtensions.primitives.set(
    'scb_putafter(first, second)',
    function (script, child) {
//if empty
...
SnapExtensions.primitives.set(
    'scb_blocksin(block)',
    function (block) { // Cleaned up, I think
        var out = [],
            inBlock = block.expression;
        while (inBlock) {
            var item = inBlock.fullCopy();
            if (item.nextBlock()) {item.children.pop(); }
            out.push(new Context(null, item));
            inBlock = inBlock.nextBlock();
        }
        return new List(out);
    }
);
...
SnapExtensions.primitives.set(
    'scb_numslots(block)',
    function (block) {
        return block.expression.inputs().length;
    }
);
...
SnapExtensions.primitives.set(
    ...
    function (block) {
        block = (block.expression? 
                    (block.expression instanceof Array? 
                        block.expression[0]:
                        block.expression)
                    :block).fullCopy();
